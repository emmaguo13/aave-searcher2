...
contract AaveSearcher is FlashLoanSimpleReceiverBase {
    ...
    event Profit(uint256 _profit);

    constructor(IPoolAddressesProvider _poolProvider, address _swapRouterAddr)
        public
        FlashLoanSimpleReceiverBase(_poolProvider)
    {
        swapRouterAddr = _swapRouterAddr;
        swapRouter = IV3SwapRouter(_swapRouterAddr);
        console.log(
            "Deploying an AaveSearcher with Swap Router Addr:",
            _swapRouterAddr
        );
    }
    ...
    */
    function executeOperation(
        ...
        (
            ...
            uint24 poolFee,
            address liquidator
        ) = abi.decode(params, (address, address, uint256, uint24, address));
        ...
        uint256 bonus = IERC20(asset).balanceOf(address(this)) -
            amount -
            premium;

        // Custom error handling is more gas efficient than require statements
        if (bonus == 0) {
            return false;
        }
